...
def kv_bytes_total_mla(batch, context_length, n_layers, latent_dim, bytes_per_elem):
    per_layer = batch * context_length * latent_dim * bytes_per_elem
    return per_layer * n_layers
...
def main():
    ...
    p.add_argument("--latent_dim", default=0, type=int)
    ...
    # 4261/5261 implement the mem size for gqa
    ...
    gqa_ratio = total_mha / total_gqa
    gqa_savings = 1 - (total_gqa / total_mha)
    mla_ratio = total_mha / total_mla if total_mla > 0 else "N/A"
    mla_savings = 1 - (total_mla / total_mha)
    ...
    print(f"MLA latent dim   : {args.latent_dim}")
    print()
    ...
    print(f"Ratio (MHA / GQA)   : {gqa_ratio:,.2f}x")
    print(f"Savings (GQA vs MHA): {gqa_savings*100:,.2f}%")
    print(f"Ratio (MHA / MLA)   : {mla_ratio:,.2f}x")
    print(f"Savings (MLA vs MHA): {mla_savings*100:,.2f}%")
